...
function OverDamping(
	...
	v0: number,
	f: number
): DifEqFunctionTable -- Two solutions r1 and r2; normal solution for second order DE
...
function CriticalDamping(
	...
	v0: number,
	f: number
): DifEqFunctionTable -- Repeated solution; must add a multiple of t
	local r = -a / 2
...
function UnderDamping(
	...
	v0: number,
	f: number
): DifEqFunctionTable -- Imaginary solution turned into sin + cos using e^(ix) = cos(x) + isin(x)
	local delta = a * a - 4 * k / m
	local r = -a / 2
