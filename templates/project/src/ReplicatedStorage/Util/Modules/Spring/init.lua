...
SpringFunctions.__tostring = function(self: SpringObject): string
	...
	if aose == false then
		formattedString =
			string.format(SPRING_PROPERTIES_FORMAT_STRING_BASIC, F.Offset(t), F.Velocity(t), F.Acceleration(t))
...
function Spring.new(
	...
	v0: number?,
	goal: number?
): SpringObject -- using a second order differential equation
